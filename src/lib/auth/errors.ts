export function authErrorMessage(raw: string | null | undefined): string {
  if (!raw) return "Google sign-in did not finish. Try again.";
  ...
  if (/database error saving new user/i.test(text)) {
    return "Google signed in, but creating the shop profile failed. Check the handle_new_user trigger in Supabase.";
  }
  return text;
}
